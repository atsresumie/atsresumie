...
export async function POST(request: NextRequest) {
	try {
		...
		// Build object path - use temp/ prefix for soft-commit flow
		// Files are committed to final/ when user clicks "Preview & Analyze"
		const bucket = "user-resumes";
		const timestamp = Date.now();
		const uuid = crypto.randomUUID().slice(0, 8);
		const sanitizedName = sanitizeFilename(filename);
		const objectPath = `sessions/${sessionId}/temp/${timestamp}-${uuid}-${sanitizedName}`;
